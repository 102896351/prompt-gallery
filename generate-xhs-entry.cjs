...
// Image: user will place real file at public/prompts/ride-giant-pink-flying-pig-1.jpg
// For now using fallback.svg placeholder
const fs = require('fs');
...
// 图片：先放 fallback（已存在），后续替换为 /prompts/ride-giant-pink-flying-pig-1.jpg
const imageSrc = '/fallback.svg';

const source = {
  platform: 'Xiaohongshu',
  sourceUrl: 'https://www.xiaohongshu.com/explore/6a019ca30000000008001eac',
  statusId: null,
  authorName: '红猫杂货店',
};
...
const entry = `  {
    ...
    images: [{"src":"${imageSrc}","alt":"${title}","width":500,"isThumb":false}],
    source: {"platform":"${source.platform}","sourceUrl":"${source.sourceUrl}","statusId":null,"authorName":"${source.authorName}"},
    dateAdded: "2026-08-06",
    tags: ["fresh","community","xiaohongshu","concept-art"],
    verdict: "Community submission from Xiaohongshu. Creative fairy-tale portrait with a pink flying pig.",
    reusable: false,
    language: "zh",
    structuredData: null,
  }`;

fs.writeFileSync('new-xhs-entry.txt', entry, 'utf8');
console.log('Entry generated and saved to new-xhs-entry.txt');
console.log('');
console.log('Image path:', imageSrc, '(fallback - real image will be at /prompts/ride-giant-pink-flying-pig-1.jpg once user places it)');